...
class AutonomousOrchestrator:
    ...
    def _build_system_prompt(self) -> str:
        ...
        system += "\n\n" + (
            ...
            "You are running in autonomous mode. You MUST use tools to do work.\n"
            "CRITICAL: Do NOT respond with just text. EVERY response MUST include at least one tool call.\n"
            "Do NOT greet the user or ask clarifying questions — immediately start executing.\n"
            "1. Plan your approach, then EXECUTE using tools — never just describe what you would do\n"
            "2. Use report_progress to keep the user informed\n"
            "3. IMPORTANT: When the user asks you to spin up agents, research something, "
            "or do parallel work, you MUST call delegate_to_subagent. Call it multiple "
            "times in a SINGLE turn to launch agents in PARALLEL. Each sub-agent runs "
            "in the background with its own tools.\n"
            "4. After delegating, use check_subagents to poll for results\n"
            ...
            "8. You can continue working while sub-agents run — do other tasks or "
            "wait for their results with check_subagents"
        )
